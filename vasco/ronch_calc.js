function drawOverlays(
    ...
    let scalar = 256;
    let keV = 300; //Number(document.getElementById("beamvolt").value);
...
function drawEverything(
    ...
    if (ab_cor_hard == 'easy' || ab_cor_mode == false) {
    ...
    if (draw_overlay) {
        drawOverlays(ctx1, ctx2, numPx, al_max, disp_size_mrad, obj_ap_r, rmax, best_r, ctx3);
    }
    }
...
function TrainingModeEasy(){
    ...
    if(switch_easy.checked){
        ...
        for (let it = 0; it < terms; it++) {
            let aberration = aberrations[it];
            aberration.mag_el.value = ab_mags[it]/aberration.mag_unit;
            if (aberration.arg_el) {
                aberration.arg_el.value = ab_angles[it] / deg;
            }
            //console.log(aberration);
        }
        calculate();
